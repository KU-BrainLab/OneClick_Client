...
def get_psd_topogrphy(epoch_data, uuid):
    ...
    for i in range(5):
        ...
        for band_name, band_range in bands.items():
            ...
            for psds_mean_sample in psds_mean:
                bp = simps(psds_mean_sample[idx_band], dx=freq_res)
...
def get_psd_analyis(epoch_data):
    def get_related_power(psds_, freqs_, freq_range):
        ...
        bp = simps(psds_[idx_band], dx=freq_res)
        bp /= simps(psds_, dx=freq_res)
        return bp
...
def get_region_psd(epoch_data):
    ...
    bands = {'delta': [0, 4], 'theta': [4, 8], 'alpha': [8, 13], 'beta': [13, 30], 'gamma': [30, 40]}
    r_brain_region = ['Fp2', 'F8', 'F4', 'C4', 'T4', 'P4', 'O2']
    l_brain_region = ['Fp1', 'F7', 'F3', 'C3', 'T3', 'P3', 'O1']
...
def get_sleep_staging(epoch_data, ch_list):
    ...
    for i in range(5):
        ...
        ckpt_path = r'D:\Project\BioSignal_Analysis_cli\neuronet\ckpt\\' + str(i) + r'\model\best_model.pth'
...
def main_anlaysis(path):
    ...
    file1 = get_psd_topogrphy(epoch_data, myuuid)
    file2 = get_brain_connectivity(epoch_data, myuuid)
    psd_result = get_psd_analyis(epoch_data)
